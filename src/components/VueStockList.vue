...
<div>
    ...
    <el-table :data="tableData" style="width: 100%" :default-sort = "{prop: 'increase', order: 'descending'}">
        ...
        <el-table-column prop="upDownRate" sortable label="上下比例">
        </el-table-column>
    </el-table>
</div>
</template>
...
export default {
    ...
    methods: {
        startFilter() {
            ...
            api.startFilter(codes,dateOffset, (result) => {
                this.totalLength = result.totalLength;
                this.remainLength = result.remainLength;
                this.filterProgress = 100 - parseInt(result.remainLength * 100 / result.totalLength);
                if (result.isGood && (selectedDate - result.stockInfo[0])<3600000 * 7 * 24) {
                    this.tableData.push({
                        companyName: result.code.name,
                        pirce: result.stockInfo[5],
                        increase: result.stockInfo[7] + "%",
                        upDownRate:result.upDownRate.toFixed(2)
                    });
                }
            });
